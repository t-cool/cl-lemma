...
(defvar *DATA_DIR* nil)
(defvar *WN_FILES*
  '((:noun (DATA_DIR '/dict/index.noun'
	   DATA_DIR + '/dict/noun.exc'))
    (:verb (DATA_DIR '/dict/index.verb'
        DATA_DIR '/dict/verb.exc'))
    (:adj (DATA_DIR '/dict/index.adj'
	   DATA_DIR + '/dict/adj.exc'))
    (:adv (DATA_DIR + '/dict/index.adv'
	   DATA_DIR + '/dict/adv.exc'))))

(defvar *kana* (loop for line in
		  (uiop:read-file-lines
		   (merge-pathnames "dict/kana.tsv"
				    (asdf:system-source-directory "ej")))
		unless (eql (aref line 0) #\#)
		collect (uiop:split-string line :separator '(#\Tab))))

In [1]: from nltk.stem import WordNetLemmatizer                        
In [2]: wordnet_lemmatizer = WordNetLemmatizer()                       
In [3]: wordnet_lemmatizer.lemmatize(“dogs”)                           
Out[4]: ‘dog’
